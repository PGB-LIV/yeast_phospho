...
# GSB list (but can also loop through the g and gsb backgrounds)
file_list<-c("01_motif_and_pathway_analysis/inputs/gold_motif_seqs.txt")
#file_list<-c("motif_analysis_step1/gold_motif_seqs.txt")
for (f in file_list){
  ...
  for (motif_group in groups){
    ...
    val <- mot_filtered$motif
    # Filter foreground by motifs in the motif group
    m <- filter(foreground, grepl(paste(val, collapse='|'),V1))
    # seq
    m <- m$V1
    
    # csv
    #write.table(m, file = paste0("01_motif_and_pathway_analysis/outputs/",f,"/",motif_group,"_15mer_",f,".txt"),quote=FALSE,row.names = FALSE,col.names = FALSE)
    # seq logo
    motif_plot <- ggplot()+geom_logo(m, method="probability")+scale_x_continuous(labels=pos,breaks=break_list)+ggtitle(motif_group)+theme(legend.position = "none")
    ...
    foreground_motif <- filter(foreground, grepl(paste(val, collapse='|'),V1))
    foreground_motif$motif <- motif_group
    colnames(foreground_motif)<-c("15mer", "Protein", "PTM_position", "UniProt_accession", "Motif group")
    ...
    result_simplified <- clusterProfiler::simplify(enrich_result, 
                                                   by = "p.adjust",
                                                   cutoff = 0.6,
                                                   select_fun = min)
    ...
    results_filtered <- filter(result_simplified, p.adjust < 0.01, qvalue < 0.2)
    ...
    dotplot_fig <- clusterProfiler::dotplot(results_filtered, title=motif_group,split="ONTOLOGY",showCategory=5)+facet_grid(ONTOLOGY ~.,scale="free")
    ...
    plots2[[counter]] <- dotplot_fig
    
    # to dataframe
    results_filtered <- as.data.frame(results_filtered@result)
    # add motif group
    results_filtered$motif <- motif_group
    
    # add to overall dataframe for enrichment analysis after simplify
    GO_simplified <- rbind(GO_simplified,results_filtered)
    cp_output <- paste("01_motif_and_pathway_analysis/outputs/",f,"/GO_simplified_",motif_group, "_",f,".csv", sep="")
    #write.csv(results_filtered, cp_output, row.names=F)
    counter = counter+1
  }
  ...
  p2<-grid.arrange(grobs=plots,layout_matrix=layout)
  p3<-grid.arrange(grobs=plots2,layout_matrix=layout)
  
  ggsave(file=paste0("01_motif_and_pathway_analysis/outputs/",f,"/rmotifx_all_",f,".png"),p2, height=10,width=7,dpi=330)
  ggsave(file=paste0("01_motif_and_pathway_analysis/outputs/",f,"/simplify_cp_all_",f,".png"),p3,dpi=330,height=30, width=16)
  ...
  for (ontology_group in GO_ont){
    ...
    # unique Go ids from simplified df
    unique_go_id_values <- unique(GO_simplified_ont[['ID']])
    ...
    # heatmap plot
    output=paste0("01_motif_and_pathway_analysis/outputs/",f,"/simplify_heatmap_",ontology_group,"_",f,".png")
